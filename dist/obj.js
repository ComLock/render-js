...
"use strict";
Object.defineProperty(exports,'__esModule',{value:true});exports.modifyStyleAndMediaToClassAndCss=modifyStyleAndMediaToClassAndCss;exports.render=render;var _util=__webpack_require__(27);var _css=__webpack_require__(33);var _html=__webpack_require__(35);var _svg=__webpack_require__(11);function _defineProperty(obj,key,value){if(key in obj){Object.defineProperty(obj,key,{value:value,enumerable:true,configurable:true,writable:true})}else{obj[key]=value}return obj}var HTML_AND_SVG_ELEMENTS=_html.ELEMENTS.concat(_svg.SVG_NOT_HTML_ELEMENTS);HTML_AND_SVG_ELEMENTS.forEach(function(t){exports[t]=function(){var _obj3;for(var _len=arguments.length,args=Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key]}if((0,_util.isString)(args[0])||(0,_util.isArray)(args[0])||!(0,_util.isSet)(args[0])||args[0]._t){var _obj2;var _obj=(_obj2={},_defineProperty(_obj2,t,{c:args[0]}),_defineProperty(_obj2,'_t',t),_obj2);return _obj}var el={a:args[0]};if((0,_util.isSet)(args[1])){el.c=args[1]}if(el.a._s){el.s=el.a._s;delete el.a._s}if(el.a._m){el.m=el.a._m;delete el.a._m}var obj=(_obj3={},_defineProperty(_obj3,t,el),_defineProperty(_obj3,'_t',t),_obj3);return obj}});function modifyStyleAndMediaToClassAndCss(view){if((0,_util.isString)(view)){return view}var arr=(0,_util.isArray)(view)?view:[view];var ret=arr.map(function(item){if((0,_util.isString)(item)){return item}var tag=Object.keys(item)[0];var value=item[tag];if((0,_util.isObject)(value)&&!(0,_util.isEmptyObject)(value)){if(value.s){var s=(0,_css.classAppendAndCssFromStyle)(value.s);item[tag].css=(0,_util.isArray)(item[tag].css)?item[tag].css.concat(s.css):s.css;if(!(0,_util.isSet)(item[tag].a)){item[tag].a={}}item[tag].a.class=item[tag].a.class?[].concat(item[tag].a.class,s.classAppend):s.classAppend;delete item[tag].s}if(value.m){var m=(0,_css.classAppendAndCssFromMedia)(value.m);item[tag].css=(0,_util.isArray)(item[tag].css)?item[tag].css.concat(m.css):m.css;if(!(0,_util.isSet)(item[tag].a)){item[tag].a={}}item[tag].a.class=item[tag].a.class?[].concat(item[tag].a.class,m.classAppend):m.classAppend;delete item[tag].m}if(value.c){modifyStyleAndMediaToClassAndCss(value.c)}}return item});return ret.length===1?ret[0]:ret}function render(view){if((0,_util.isString)(view)){return{css:[],html:view}}var res={css:[],html:''};var arr=(0,_util.isArray)(view)?view:[view];for(var i=0;i<arr.length;i+=1){var item=arr[i];if((0,_util.isString)(item)){res.html+=item}else{var tag=Object.keys(item)[0];var boolVoid=(0,_html.isVoid)(tag);var value=item[tag];var attrStr='';var contentStr='';if((0,_util.isArray)(value)){throw new Error('The value of tag:'+tag+' cannot be of type array!')}else if((0,_util.isFunction)(value)){throw new Error('The value of tag:'+tag+' cannot be of type function!')}else if((0,_util.isObject)(value)){if((0,_util.isEmptyObject)(value)){contentStr=''}else{if(!boolVoid&&value.c){var c=render(value.c);res.css=res.css.concat(c.css);contentStr=c.html}var attrs=value.a||{};if(value.css){res.css=res.css.concat(value.css)}if(value.s){var s=(0,_css.classAppendAndCssFromStyle)(value.s);res.css=res.css.concat(s.css);attrs.class=attrs.class?[].concat(attrs.class,s.classAppend):s.classAppend}if(value.m){var m=(0,_css.classAppendAndCssFromMedia)(value.m);res.css=res.css.concat(m.css);attrs.class=attrs.class?[].concat(attrs.class,m.classAppend):m.classAppend}if(res.css){res.css=(0,_css.uniqCss)(res.css)}if(attrs){attrStr=(0,_html.att2Str)(attrs)}}}else if((0,_util.isString)(value)){contentStr=value}if(boolVoid){res.html+='<'+tag+attrStr+'/>'}else{res.html+='<'+tag+attrStr+'>'+contentStr+'</'+tag+'>'}}}return res}
