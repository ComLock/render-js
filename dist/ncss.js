...
"use strict";
Object.defineProperty(exports,'__esModule',{value:true});var _index=__webpack_require__(37);var _svg=__webpack_require__(11);var _util=__webpack_require__(27);var _css=__webpack_require__(33);function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError('Cannot call a class as a function')}}var Node=function Node(){var _ref=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{},_ref$html=_ref.html,html=_ref$html===undefined?'':_ref$html,_ref$css=_ref.css,css=_ref$css===undefined?{}:_ref$css;_classCallCheck(this,Node);this.html=html;this.css=css};exports.Node=Node;exports.el=function(tag){var spec=arguments.length>1&&arguments[1]!==undefined?arguments[1]:null;var content=arguments.length>2&&arguments[2]!==undefined?arguments[2]:null;var _ref2=arguments.length>3&&arguments[3]!==undefined?arguments[3]:{},_ref2$autoprefixer=_ref2.autoprefixer,autoprefixer=_ref2$autoprefixer===undefined?true:_ref2$autoprefixer;if(spec instanceof Node||(0,_util.isArrayOrFuncOrString)(spec)){content=spec;spec={}}var css=[];if(spec){if(spec.style){var s=(0,_css.classAppendAndCssFromStyle)(spec.style,{autoprefixer:autoprefixer});spec.class=[].concat(spec.class,s.classAppend).filter(function(n){return n});css=(0,_css.uniqCss)(css.concat(s.css));spec.style=null}if(spec._media){var o=(0,_css.classAppendAndCssFromMedia)(spec._media,{autoprefixer:autoprefixer});spec.class=[].concat(spec.class,o.classAppend).filter(function(n){return n});css=(0,_css.uniqCss)(css.concat(o.css));spec._media=null}}if(!(0,_util.isArray)(content)){content=[content]}content=content.map(function(item){while((0,_util.isFunction)(item)){item=item()}if(item instanceof Node){css=(0,_css.uniqCss)(css.concat(item.css));return item.html}if((0,_util.isString)(item)){return item}return''}).join('');return new Node({html:(0,_index.el)(tag,spec,content),css:css})};var HTML_AND_SVG_ELEMENTS=_index.ELEMENTS.concat(_svg.SVG_NOT_HTML_ELEMENTS);HTML_AND_SVG_ELEMENTS.forEach(function(k){exports[k]=function(){var _exports;for(var _len=arguments.length,args=Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key]}return(_exports=exports).el.apply(_exports,[k].concat(args))}});exports.default=exports;
